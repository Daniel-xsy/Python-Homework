...
class mainPage(object):
    ...
    def searchPage(self):
        search_lable=ttk.Label(self.root,text='请输入学生姓名或学号')
        search_entry=ttk.Entry(self.root,textvariable=self.search_input)
        search_button=ttk.Button(self.root,text='确认',command=self.searchGrade)
        ...
        with open('students.csv','r',encoding='utf-8') as file:
            ...
            for row in reader:
                if row[0]==string or row[1]==string:
                    ...
                    self.infor_frame.grid(row=4,column=1,rowspan=2,columnspan=4)
                    name=str(row[1])
                    id=str(row[0])
                    grade=str(row[4:])
                    ...
                    id_lable.grid(row=1,column=1)
                    grade_lable.grid(row=1,column=3,columnspan=3)
                    return
